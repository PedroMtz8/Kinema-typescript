...
export const getMovies = async (page: string) => {
  const results = await axios.get(`https://api.themoviedb.org/3/movie/popular?api_key=${YOUR_API_KEY_1}&language=en-US&page=${page}`)
 ...
 /*  if (results === undefined) {
  ...
  const movieData = results.map((m: any) => {
    return {
      id: m.id,
      title: m.title,
